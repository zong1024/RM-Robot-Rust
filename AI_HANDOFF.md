...
正式底盘要求 ID1～4 全部在线。任一反馈超过 20 ms 未更新会整体锁零并清 PID/软启动状态。
...
CAN 硬件过滤器只接收六个目标反馈 ID，软件还会拒绝扩展帧、远程帧和非 8 字节帧。
单次 CAN 中断最多处理 8 帧。控制循环间隔超过 5 ms 时整车锁零并要求重新解锁；
主循环卡死约 500 ms 后独立看门狗复位。
...
云台默认未标定，`GIMBAL_CALIBRATION.calibrated` 为 `false` 时始终输出零电流。
必须填写俯仰机械零点和方向、架空验证后才能启用。底盘不受云台标定状态影响。
