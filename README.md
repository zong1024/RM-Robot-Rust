...
轮胎模式由 FS-i6 的两档 `SwA` 选择：

| SwA 档位 | 模式 | 左摇杆竖直 | 左摇杆水平 |
| --- | --- | --- | --- |
| 上档 | 普通轮 | 前后 | 转向 |
| 下档 | 麦克纳姆轮 | 前后 | 横移 |

SwA 默认映射到 CH7（`channels[6]`）。模式切换时底盘会清空速度 PID，并输出
...
通过 SWD 可查看 `SWA_CHANNEL_RAW` 和 `CHASSIS_WHEEL_MODE`：后者为 `0` 时是
普通轮模式，为 `1` 时是麦克纳姆轮模式。
